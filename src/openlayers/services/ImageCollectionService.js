...
import CommonMatchImageCollectionService from '@supermapgis/iclient-common/iServer/ImageCollectionService';
...
export class ImageCollectionService extends ServiceBase {
     ...
     * @description 删除影像集合中的指定 ID（`featureId`）的 Item 对象，即从影像集合中删除指定的影像。
     ...
     * @description 返回影像集合中的指定 ID（`featureId`）的 Item 对象，即返回影像集合中指定的影像。
     * @param {string} featureId Feature 的本地标识符。
     * @param {RequestCallback} [callback] 回调函数，该参数未传时可通过返回的 promise 获取结果。
     * @returns {Promise} Promise 对象。
     */
