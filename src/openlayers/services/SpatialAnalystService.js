...
export class SpatialAnalystService extends ServiceBase {
     ...
     * @param {InterpolationRBFAnalystParameters|InterpolationDensityAnalystParameters|InterpolationIDWAnalystParameters|InterpolationKrigingAnalystParameters} params - 插值分析参数类，包括：点密度插值分析参数类、反距离加权插值分析参数类、样条插值（径向基函数插值法）分析参数类、克吕金插值分析参数类。
     * @param {RequestCallback} [callback] 回调函数，该参数未传时可通过返回的 promise 获取结果。
     * @param {DataFormat} [resultFormat=DataFormat.GEOJSON] - 返回的结果类型。
     * @returns {Promise} Promise 对象。
     */
     ...
     * @description 表面分析。<br>
     * 表面分析主要通过生成新数据集，获得更多反映原始数据集中所暗含的空间特征、空间格局等信息。
