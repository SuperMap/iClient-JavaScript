import cv from '../../../libs/opencv-js/opencv_js';

/**
 * @class SuperMap.CoordTransfer
 * @classdesc 坐标转换
 ...
 * @param {number} [options.fx] - 相机水平焦距。
 * @param {number} [options.fy] - 相机垂直焦距。
 * @param {number} [options.cx] - 相机中心的水平坐标。
 * @param {number} [options.cy] - 相机中心的垂直坐标。
 */
...
export default class CoordTransfer {
  ...
  /**
   * @function SuperMap.CoordTransfer.prototype.setCameraLocation
   ...
   * @param {Object} [options.cx] - 相机中心的水平坐标。
   * @param {Object} [options.cy] - 相机中心的垂直坐标。
   */
  setCameraLocation(configs) {
    const { pitch, roll, yaw, x, y, z, fx, fy, cx, cy } = configs;
    if (!this.cv) {
      return;
    }
    this.rotationMatrix = this.toRotationMatrix(pitch, roll, yaw);
    this.translationMatrix = this.toTranslationMatrix(x, y, z);
    this.k = this.toCameraMatrix(fx, fy, cx, cy);
  }

  /**
   * @function SuperMap.CoordTransfer.prototype.toRotationMatrix
   * @description  计算旋转矩阵。
  ...
  toRotationMatrix(pitch, roll, yaw) {
    ...
    let x = (90 - pitch) * (Math.PI / 180); // pitch
    let y = -1 * roll * (Math.PI / 180); // roll
    let z = -1 * yaw * (Math.PI / 180); // yaw
    ...
    this.cv.gemm(rz, ry, 1, new this.cv.Mat(), 0, tempResult);
    this.cv.gemm(tempResult, rx, 1, new this.cv.Mat(), 0, tempResult);
    this.cv.Rodrigues(tempResult, rotationMatrix);
    return rotationMatrix;
  }

  /**
   * @function SuperMap.CoordTransfer.prototype.toTranslationMatrix
   * @description  计算偏移矩阵
  ...
  /**
   * @function SuperMap.CoordTransfer.prototype.toCameraMatrix
   * @description  计算相机矩阵。
   * @param {Object} fx - 水平焦距。
   * @param {Object} fy - 垂直焦距。
   * @param {Object} cx - 相机中心的水平坐标
   * @param {Object} cy - 相机中心的垂直坐标。
   * @returns {Array} 视频矩阵。
   */
  toCameraMatrix(fx, fy, cx, cy) {
    return this.cv.matFromArray(3, 3, this.cv.CV_64FC1, [fx, 0, cx, 0, fy, cy, 0, 0, 1]);
  }
  /**
   * @function SuperMap.CoordTransfer.prototype.toVideoCoordinate
  ...
  /**
   * @function SuperMap.VideoMap.prototype.toSpatialCoordinate
   * @description  转换视频像素坐标到空间地理坐标
