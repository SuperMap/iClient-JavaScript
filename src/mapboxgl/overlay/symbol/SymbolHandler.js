...
class SymbolHandler {
     ...
     */
    getLayer(layerId) {
        const layer = this.map.style.getLayer(layerId);
        ...
        if (layer) {
            ...
            if (layerIds?.[0]) {
                const reallayer = this.map.style.getLayer(layerIds[0]);
    ...
    moveLayer(layerId, beforeId) {
        const layerIds = this.getLayerIds(layerId);
        if (layerIds?.length > 0) {
            layerIds.forEach(id => this.map.style.moveLayer(id, beforeId));
        } else {
            this.map.style.moveLayer(layerId, beforeId);
        }
    }
