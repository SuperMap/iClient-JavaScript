 ...
 * @param {Array.<SuperMap.LayerStatus>} options.layerStatusList - 获取或设置图层可见状态（{@link SuperMap.LayerStatus}）集合，
 *                                                                 集合中的每个 {@link SuperMap.LayerStatus} 对象代表一个子图层的可视状态。
 * @param {string} [options.holdTime=15] - 获取或设置资源在服务端保存的时间。
...
export class SetLayerStatusParameters {
    ...
    constructor(options) {
         ...
         * @description 获取或设置图层可见状态（SuperMap.LayerStatus）集合，<br>
         ...
         * @description 获取或设置资源服务ID。如果设置该参数则会在指定的 TempLayer 进行图层的显示控制；<br>
