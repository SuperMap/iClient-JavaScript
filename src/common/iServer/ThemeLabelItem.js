 ...
 * 分段标签专题图允许用户通过 rangeExpression 字段指定用于分段的数值型字段，
 * 同一范围段内的标签具有相同的显示风格，其中每一个范围段就是一个专题图子项，
 * 每一个子项都具有其名称、风格、起始值和终止值。<br>
 * 注意：每个分段所表示的范围为 [Start, End)。例如：标签专题图的分段点有两个子项，
 ...
 * @param {string} [options.caption] - 分段标签专题图子项的名称。
 * @param {number} [options.end=0] - 分段标签专题图子项的终止值。
 * @param {number} [options.start=0] - 分段标签专题图子项的分段起始值。
 * @param {boolean} [options.visible=true] - 分段标签专题图子项是否可见。
 * @param {ServerTextStyle} [options.style] - 分段标签专题图子项文本的显示风格。
 * @usage
 */
export class ThemeLabelItem {
    ...
    constructor(options) {
         ...
         * @description 分段标签专题子项的标题。
         */
        this.caption = null;

        /**
         * @member {number} [ThemeLabelItem.prototype.end=0]
         * @description 分段标签专题图子项的分段终止值。如果该子项是分段中最后一个子项，那么该终止值就是分段的最大值；
         *              如果不是最后一项，该终止值必须与其下一子项的起始值相同，否则系统会抛出异常。
         */
        this.end = 0;

        /**
         * @member {number} [ThemeLabelItem.prototype.start=0]
         * @description 分段标签专题图子项的分段起始值。如果该子项是分段中第一个子项，那么该起始值就是分段的最小值；
         ...
         * @description 分段标签专题图子项是否可见。如果为 true，则标签专题图子项可见。
         */
        this.visible = true;

        /**
         * @member {ServerTextStyle} ThemeLabelItem.prototype.style
         * @description 分段标签专题图子项文本的显示风格。各种风格的优先级从高到低为：<br>
