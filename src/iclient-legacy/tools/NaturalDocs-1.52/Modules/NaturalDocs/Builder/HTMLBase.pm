...
sub EndBuild #(hasChanged)
	...
	my $mergeDivOutput = $self->MergeDiv();
	my $mergeJQueryOutput = $self->MergeJQuery();
	my $mergeBootStrapOutput = $self->MergeBootStrap();
    ...
    if (NaturalDocs::Settings->HighlightCode() || NaturalDocs::Settings->HighlightAnonymous())
		...
		my $mergeDivMaster = NaturalDocs::File->JoinPaths( NaturalDocs::Settings->JavaScriptDirectory(), 'Merge.js' );
		my $mergeJQueryMaster = NaturalDocs::File->JoinPaths( NaturalDocs::Settings->JavaScriptDirectory(), 'JQuery.js' );
		my $mergeBootStrapMaster = NaturalDocs::File->JoinPaths( NaturalDocs::Settings->JavaScriptDirectory(), 'BootStrap.js' );
	    # We check both the date and the size in case the user switches between two styles which just happen to have the same
	    # date.  Should rarely happen, but it might.
	    if (! -e $prettifyOutput ||
			...
			NaturalDocs::File->Copy($mergeDivMaster, $mergeDivOutput);
			NaturalDocs::File->Copy($mergeJQueryMaster, $mergeJQueryOutput);
			NaturalDocs::File->Copy($mergeBootStrapMaster, $mergeBootStrapOutput);
	        };
    ...
    if (!exists $prebuiltMenus{$outputDirectory})
        ...
        if (scalar keys %{NaturalDocs::Menu->Indexes()})
            ...
            . '<div id=MSearchPanel class=MSearchPanelInactive>'
                . '<input type=text id=MSearchField placeholder=Search '
...
sub BuildHeader
	...
	my $self = shift;
	my $root = NaturalDocs::Settings->OutputDirectoryOf($self);
	my $baseURL = $self->MakeRelativeURL($outputFile, $root, 1);
    my $output = '<div class="navbar navbar-fixed-top" id="navbar"></div><script src="' . $baseURL . '/../examples/js/navbar.js"></script></script><script src="' . $baseURL . '/../examples/js/GoTop.js" id="js_gotop"></script>';
    return $output;
    };
...
sub MergeDiv
	{
	my $self = shift;
	return NaturalDocs::File->JoinPaths( $self->JavaScriptDirectory(), 'merge.js' );
	};

sub MergeJQuery
	{
	my $self = shift;
	return NaturalDocs::File->JoinPaths( $self->JavaScriptDirectory(), 'jquery.js' );
	};
	
sub MergeBootStrap
	{
	my $self = shift;
	return NaturalDocs::File->JoinPaths( $self->JavaScriptDirectory(), 'bootstrap.js' );
	};
