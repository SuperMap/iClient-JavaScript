module("UniqueLayer");
var name = "Unique layer";

test("TestUnique_constructor",function () {
    expect(7);
    var themeLayer = new SuperMap.Layer.Unique(name);

    ok(themeLayer instanceof SuperMap.Layer.Theme, "layer instanceof SuperMap.Layer.Unique");
    equals(themeLayer.name, name , "the name of test");
    equals(themeLayer.CLASS_NAME, "SuperMap.Layer.Unique", "CLASS_NAME");
    ...
    equals(themeLayer.isHoverAble,false, "default_isHoverAble");
    themeLayer.destroy();
});


asyncTest("TestUnique_addFeatures", function () {
    var themeLayer = new SuperMap.Layer.Unique(name, {isBaseLayer: true});
...
test("TestUnique_getFeaturesByAttribute", function () {
    expect(4);
    var themeLayer = new SuperMap.Layer.Unique(name, {isBaseLayer: true});
...
test("TestUnique_getFeaturesBy", function () {
    expect(2);
    var layer = new SuperMap.Layer.Unique(name, {isBaseLayer: true});
    ...
    equals(layer.getFeatureBy("fid", "f_01"), null,
        "测试在没有添加feature时候使用这个方法");
    layer.addFeatures([feature_1, feature_3]);
    equals(layer.getFeatureBy("fid", "f_01").id, feature_1.id,
        "测试在添加三个feature后，查找属性名为fid属性值为f_01的feature");
});

test("TestUnique_removeFeatures", function () {
    expect(2);
...
test("TestUnique_getFeaturesBy", function () {
    ...
    equals(themeLayer.getFeatureBy("fid", "f_01"), null,
        "测试在没有添加feature时候使用这个方法");
    themeLayer.addFeatures([feature_1, feature_2]);
    equals(themeLayer.getFeatureBy("fid", "f_01").id, feature_1.id,
        "测试在添加2个feature后，查找属性名为fid属性值为f_01的feature");
});

test("TestUnique_getFeaturesById", function () {
    expect(2);
...
test("TestUnique_getFeatures", function () {
    expect(2);
...
test("TestUnique_destroy", function () {
    expect(7);
    ...
    themeLayer.addFeatures([pointFeature]);
    themeLayer.destroy();
    //SuperMap.Layer.Unique的destroy方法
...
test("TestUnique_setOpacity",function(){
    expect(1);
...
test("TestUnique_setMaxCacheCount",function () {
    expect(2);
    ...
    equals(themeLayer.cacheFields.length,themeLayer.features.length,"不设置最大缓存数时，测试的最大缓存数");
    themeLayer.clearCache();

    themeLayer.setMaxCacheCount(3);
    themeLayer.addFeatures(arr);
    equals(themeLayer.cacheFields.length,3,"设置缓存数为3时，测试的最大缓存数");
    themeLayer.destroy();
    map.destroy();
});

test("TestUnique_nodesClipPixel",function () {
    expect(2);
...
test("TestUnique_styleGroups",function () {
    expect(2);
    ...
    themeLayer.addFeatures([feature_1,feature_2,feature_3]);
    equals(themeLayer.renderer.getAllShapes()[0].style.r,3,"设置value值后，测试风格分组数组style是否设置成功");
    equals(themeLayer.renderer.getAllShapes()[2].style.r,6,"未设置value值，测试风格分组数组style是否为默认值");
    themeLayer.destroy();
    map.destroy();
});
