...
/**
 * @requires SuperMap/BaseTypes/Date.js
...
/**
 * Class: SuperMap.Format.KML
 * KML解析器。读取或者写入KML数据。
 *
 * Inherits from:
 *  - <SuperMap.Format.XML>
 */
SuperMap.Format.KML = SuperMap.Class(SuperMap.Format.XML, {
     ...
     * {Boolean} 只有当 extractAttributes 为 true时才会用到此属性。默认为false。
     *           如果此属性为true，则KML属性将会被简单地设置为键值对，并与其他格式兼容，任何的displayName元素都会被忽略。
     *           如果此属性为false,则KML属性将会被转换为objects，所有的diaplayName元素都会被包含进来，包括在CDATA的元素，但不与其他格式兼容。
     */
    kvpAttributes: false,
    
    /**
     * Property: extractStyles
     * {Boolean} 从KML中提取styles属性，默认为false，提取styleUrls属性也设置此属性为true
     */
    extractStyles: false,
     ...
     * .   假如为true, features 将会包含所有来自 gx:Track元素的points，
     *     Features 也将会包含一个来自track元素的时间属性，如果 tracks
     *     包含有angle元素，则features将会拥有haading,tilt和roll属性。
     *     如果track point的坐标里包含三个值，则features也将从第三个值获取到“高度”属性。
     */
    extractTracks: false,
    ...
    /**
     * Constructor: SuperMap.Format.KML
     * 构造一个新的KML解析器实例。
     *
     * Parameters:
    ...
    initialize: function(options) {
        ...
        this.externalProjection = new SuperMap.Projection("EPSG:4326");
    ...
    /**
     * Method: parseData
     * 从一个字符串或者DOMElement中读取数据，并返回features数组
     * 
     * Parameters: 
     * data    - {String} or {DOMElement} 将要被字符串或者DOMElement数据
     * options - {Object} Hash of options
     *
     * Returns:
     * {Array(<SuperMap.Feature.Vector>)} features 数组.
     */
    ...
    parseLinks: function(nodes, options) {
        ...
        // increase depth
        var newOptions = SuperMap.Util.extend({}, options);
        newOptions.depth++;
    ...
    fetchLink: function(href) {
        var request = SuperMap.Request.GET({url: href, async: false});
        if (request) {
    ...
    parseStyle: function(node) {
        ...
        for(var i=0, len=types.length; i<len; ++i) {
            ...
            switch (type.toLowerCase()) {
                ...
                case "balloonstyle":
                    var balloonStyle = SuperMap.Util.getXmlNodeValue(
                                            styleTypeNode);
                    if (balloonStyle) {
            ...
            if(feature) {
                ...
                if (this.extractStyles) {
                    ...
                    if (inlineStyleNode) {
                        ...
                        if (inlineStyle) {
                            feature.style = SuperMap.Util.extend(
    ...
    readers: {
        "kml": {
            "when": function(node, container) {
                container.whens.push(SuperMap.Date.parse(
        ...
        "gx": {
            ...
            "coord": function(node, container) {
                ...
                var point = new SuperMap.Geometry.Point(coords[0], coords[1]);
                if (coords.length > 2) {
    ...
    /**
     * Method: parseFeature
     * This function is the core of the KML parsing code in SuperMap.
     ...
     *
     * Returns:
     * {<SuperMap.Feature.Vector>} A vector feature.
     */
    ...
    getStyle: function(styleUrl, options) {
        ...
        var styleBaseUrl = SuperMap.Util.removeTail(styleUrl);
        ...
        if (!this.styles[styleUrl] 
                && !SuperMap.String.startsWith(styleUrl, "#") 
        ...
        var style = SuperMap.Util.extend({}, this.styles[styleUrl]);
        return style;
    },
    ...
    parseGeometry: {
         ...
         * Given a KML node representing a point geometry, create an SuperMap
         *     point geometry.
         *
         * Parameters:
        ...
        point: function(node) {
            ...
            if(coords.length > 1) {
                ...
                point = new SuperMap.Geometry.Point(coords[0], coords[1],
                                                      coords[2]);
            } else {
            ...
            if(nodeList.length > 0) {
                ...
                for(var i=0; i<numPoints; ++i) {
                    ...
                    if(numCoords > 1) {
                        ...
                        points[i] = new SuperMap.Geometry.Point(coords[0],
                                                                  coords[1],
                                                                  coords[2]);
                    } else {
         ...
         *
         * Returns:
         * {<SuperMap.Geometry.Collection>} A geometry collection.
         */
        multigeometry: function(node) {
            ...
            return new SuperMap.Geometry.Collection(parts);
        }
                ...
                if(grandchildren.length >= 1 && grandchildren.length <= 3) {
                    ...
                    if(grandchild.nodeType === 3 || grandchild.nodeType === 4) {
                        var name = (child.prefix) ?
                                child.nodeName.split(":")[1] :
                                child.nodeName;
                        var value = SuperMap.Util.getXmlNodeValue(grandchild);
                        if (value) {
    ...
    parseProperty: function(xmlNode, namespace, tagName) {
        ...
        try {
            value = SuperMap.Util.getXmlNodeValue(nodeList[0]);
        } catch(e) {
            value = null;
        }
    ...
    write: function(features) {
        ...
        return SuperMap.Format.XML.prototype.write.apply(this, [kml]);
    },
    ...
    buildGeometry: {
         ...
         * Given an SuperMap point geometry, create a KML point.
         *
         * Parameters:
         ...
         * Given an SuperMap multipoint geometry, create a KML
         *     GeometryCollection.
         *
         * Parameters:
         * geometry - {<SuperMap.Geometry.Point>} A multipoint geometry.
         *
         * Returns:
         ...
         * Given an SuperMap linestring geometry, create a KML linestring.
         *
         * Parameters:
         ...
         * Given an SuperMap multilinestring geometry, create a KML
         *     GeometryCollection.
         *
         * Parameters:
         * geometry - {<SuperMap.Geometry.Point>} A multilinestring geometry.
         *
         * Returns:
         * {DOMElement} A KML GeometryCollection node.
         */
         ...
         * Given an SuperMap linearring geometry, create a KML linearring.
         *
         * Parameters:
         ...
         * Given an SuperMap polygon geometry, create a KML polygon.
         *
         * Parameters:
         ...
         * Given an SuperMap multipolygon geometry, create a KML
         *     GeometryCollection.
         *
         * Parameters:
         * geometry - {<SuperMap.Geometry.Point>} A multipolygon geometry.
         *
         * Returns:
         * {DOMElement} A KML GeometryCollection node.
         */
         ...
         * Given an SuperMap geometry collection, create a KML MultiGeometry.
         *
         * Parameters:
         * geometry - {<SuperMap.Geometry.Collection>} A geometry collection.
         *
         * Returns:
    ...
    CLASS_NAME: "SuperMap.Format.KML" 
});
