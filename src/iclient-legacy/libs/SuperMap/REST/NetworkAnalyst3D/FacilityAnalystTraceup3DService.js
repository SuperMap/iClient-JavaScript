...
SuperMap.REST.FacilityAnalystTraceup3DService = SuperMap.Class(SuperMap.ServiceBase, {
     ...
     * {Array(String)}
     * 此类支持的事件类型。
     * - *processCompleted* 服务端返回结果成功触发该事件。 
     * - *processFailed* 服务端返回结果失败触发该事件。       
     */
    EVENT_TYPES: [ 
     ...
     /**
     * APIProperty: events
     * {<SuperMap.Events>} 在 FacilityAnalystTraceup3DService 类中处理所有事件的对象，支持两种事件 processCompleted 、processFailed ，服务端成功返回最近设施分析结果时触发 processCompleted  事件，服务端返回最近设施分析结果失败触发 processFailed 事件。
     *     
     */ 
    events: null,
    ...
    /** 
     * Property: lastResult
     * {<SuperMap.REST.FacilityAnalystTraceup3DResult>}
     */
    lastResult: null,
    ...
    findClosestFacilityComplete: function(result) {
        var me = this,
            FacilityAnalystTraceup3DResult, fe;
        result = SuperMap.Util.transformResult(result);
        FacilityAnalystTraceup3DResult = SuperMap.REST.FacilityAnalystTraceup3DResult.fromJson(result);
        me.lastResult = FacilityAnalystTraceup3DResult;
        fe = new SuperMap.REST.FacilityAnalystTraceup3DEventArgs(FacilityAnalystTraceup3DResult, result);
