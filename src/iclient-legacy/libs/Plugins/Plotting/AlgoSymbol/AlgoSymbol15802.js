 ...
 */
SuperMap.Geometry.AlgoSymbol15802 = new SuperMap.Class(SuperMap.Geometry.AlgoSymbol15800, {
    /**
    ...
    calculateParts: function () {
        ...
        for (var i = 0; i < (nMainPts - 1); ++i) {
            ...
            //求第一个桩
            var firstPts = [], midPts = [],secondPts = [];
            var pt = SuperMap.Plot.PlottingUtil.FindPointOnLineByRatio(0.35, mainLinePts[i], mainLinePts[i + 1]);
            midPt = new SuperMap.Geometry.Point((mainLinePts[i].x + mainLinePts[i + 1].x) / 2, (mainLinePts[i].y + mainLinePts[i + 1].y) / 2);
            //求中点的垂直平分点
            var midPoints = SuperMap.Plot.PlottingUtil.getSidePointsOfLine(dLen * 0.5, mainLinePts[i], midPt);
            midPts.push(midPoints.pntLeft,midPoints.pntRight);
            //求垂直平分点
            var vecPints = SuperMap.Plot.PlottingUtil.getSidePointsOfLine(dLen * 0.5, mainLinePts[i], pt);
            firstPts.push(vecPints.pntLeft,vecPints.pntRight);
            //第一个点数组与第二个点数组是关于中点对称的
            secondPts.push(new SuperMap.Geometry.Point(midPt.x * 2 - vecPints.pntLeft.x, midPt.y * 2 - vecPints.pntLeft.y));
            secondPts.push(new SuperMap.Geometry.Point(midPt.x * 2 - vecPints.pntRight.x, midPt.y * 2 - vecPints.pntRight.y));

            //生成中间桩的折线图元
            this.addCell(SuperMap.Plot.SymbolType.POLYLINESYMBOL, midPts);
    ...
    CLASS_NAME: "SuperMap.Geometry.AlgoSymbol15802"
});