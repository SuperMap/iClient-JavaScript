 ...
 */
SuperMap.Geometry.AlgoSymbol23600 = new SuperMap.Class(SuperMap.Geometry.AlgoSymbol, {
    ...
    initialize: function (option) {
        ...
        if(this.subSymbols >= 0){
            this.subSymbols.push(new SuperMap.Plot.SubSymbol(100, 800));
            this.subSymbols.push(new SuperMap.Plot.SubSymbol(100, 800));
        }
    },
    ...
    calculateParts: function () {
        ...
        if (this.subSymbols && null !== this.subSymbols && this.subSymbols.length > 0) {
            this.computeSubSymbol(this.subSymbols[0], symbolPt1, dSymbolSize1, dSymbolAngle1);
        }
        ...
        if (this.subSymbols && null !== this.subSymbols && this.subSymbols.length > 1) {
            this.computeSubSymbol(this.subSymbols[1], symbolPt2, dSymbolSize2, dSymbolAngle2);
        }
    ...
    modifyPoint: function(index, pt) {
        if(pt.isScalePoint === true){
            ...
            if(0 == index)
            {
                var dScale6 = (scalePt2D.x-initialPt1.x)/dDistance;
                this.scaleValues[6] = dScale6;

                var dScale7 = (scalePt2D.y-initialPt1.y)/dDistance;
            ...
            else if(1 == index)
            {
                ...
                var dTempDis = SuperMap.Plot.PlottingUtil.distance(symbolPt1,scalePt2D);
                var dScale0 = dTempDis*2*0.4/dDistance;
                this.scaleValues[0] = dScale0;

                var dScale1 = SuperMap.Plot.PlottingUtil.radian(symbolPt1,scalePt2D)*180/Math.PI;
                this.scaleValues[1] = dScale1;

                if(0 <= dScale1 && dScale1 < 90)
                {
                    this.scaleValues[2] = 1;
                }
                else if(90 <= dScale1 && dScale1 < 180)
                {
                    this.scaleValues[2] = 2;
                }
                else if(180 <= dScale1 && dScale1 < 270)
                {
                    this.scaleValues[2] = 3;
                }
                else
                {
                    this.scaleValues[2] = 4;
                }
            }
            else if(2 == index)
            {
                var dScale8 = (scalePt2D.x-initialPt2.x)/dDistance;
                this.scaleValues[8] = dScale8;

                var dScale9 = (scalePt2D.y-initialPt2.y)/dDistance;
                this.scaleValues[9] = dScale9;
            }
            else if(3 == index)
            {
                var dScale8 = this.scaleValues[8];
                ...
                var dTempDis = SuperMap.Plot.PlottingUtil.distance(symbolPt2,scalePt2D);
                var dScale3 = dTempDis*2*0.4/dDistance;
                this.scaleValues[3] = dScale3;

                var dScale4 = SuperMap.Plot.PlottingUtil.radian(symbolPt2,scalePt2D)*180/Math.PI;
                this.scaleValues[4] = dScale4;

                if(0 <= dScale4 && dScale4 < 90)
                {
                    this.scaleValues[5] = 1;
                }
                else if(90 <= dScale4 && dScale4 < 180)
                {
                    this.scaleValues[5] = 2;
                }
                else if(180 <= dScale4 && dScale4 < 270)
                {
                    this.scaleValues[5] = 3;
                }
                else
                {
                    this.scaleValues[5] = 4;
                }
            }
        }
