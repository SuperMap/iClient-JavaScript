 ...
 */
SuperMap.Geometry.AlgoSymbol12500 = new SuperMap.Class(SuperMap.Geometry.AlgoSymbol, {
    ...
    calculateParts: function () {
        ...
        //第一个比例点
        this.scalePoints = [];
        var scalePoint = circleCenterPt;
        scalePoint.isScalePoint = true;
        scalePoint.tag = 0;
        this.scalePoints.push(scalePoint);

        //第二份比例点
        var scalePoint1 = new SuperMap.Geometry.Point(circleCenterPt.x + dRadius, circleCenterPt.y);
        ...
        if (pt.isScalePoint === true) {
            ...
            if (index === 0) {
                ...
                for (var i = 0, len = (geoPts.length - 1); i < len; i++) {
                    ...
                    var projectPtInfo = SuperMap.Plot.PlottingUtil.pointProjectToSegment(pt, geoPts[i], geoPts[i + 1]);
                ...
                for (var i = 0, len = nindex - 1; i < len; i++) {
                    dalllen += SuperMap.Plot.PlottingUtil.distance(geoPts[i], geoPts[i + 1]);
                }

                dalllen += SuperMap.Plot.PlottingUtil.distance(ptPlumb, geoPts[nindex]);

                var dscale = dalllen / dDistance;
            ...
            if (index === 1) {
                var scaleLine = this.scaleValues[0];
                //开始点到中点的距离
                var startToCenterDistance = scaleLine * dDistance;
                //获取圆心的坐标和线段的索引
                var result = SuperMap.Plot.PlottingUtil.findPointInPolyLine(geoPts, startToCenterDistance);
                if (-1 === result.index) {
                    return;
                ...
                var dis = SuperMap.Plot.PlottingUtil.distance(result.pt, pt);

                var dScale1 = dis / dDistance;
                if (dScale1 > 0.4) {
                    dScale1 = 0.4;
                }
    ...
    CLASS_NAME: "SuperMap.Geometry.AlgoSymbol12500"
});
