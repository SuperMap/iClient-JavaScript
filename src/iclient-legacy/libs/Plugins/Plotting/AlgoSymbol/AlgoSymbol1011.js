 ...
 */
SuperMap.Geometry.AlgoSymbol1011 = new SuperMap.Class(SuperMap.Geometry.AlgoSymbol1004,{
    ...
    initialize:function(option){
        ...
        this.scaleValues.push(this.sv_arrowTailWidth);//箭尾的宽度比例值
    },
    ...
    calculateParts: function () {
        ...
        var dTailWidth = dAllDistance*this.scaleValues[4];
        var sidepoint = SuperMap.Plot.PlottingUtil.getSidePointsOfLine(dTailWidth, geoPts[1], geoPts[0]);

        this.controlPoints = [];
        this.controlPoints.push(sidepoint.pntLeft);
        this.controlPoints.push(sidepoint.pntRight);
        for(var i = 1; i < geoPts.length; i++){
            this.controlPoints.push(geoPts[i].clone());
        }

        var scaleValueBack = [];
        for(var i = 0; i < this.scaleValues.length; i++){
            scaleValueBack.push(this.scaleValues[i]);
        }
        this.scaleValues = [];
        for(var i = 0; i < scaleValueBack.length - 1; i++){
            this.scaleValues.push(scaleValueBack[i]);
        }
        
        SuperMap.Geometry.AlgoSymbol1004.prototype.calculateParts.apply(this, arguments);

        var scaleTailpoint = sidepoint.pntRight.clone();
        scaleTailpoint.isScalePoint = true;
        scaleTailpoint.tag = 3;
        this.scalePoints.push(scaleTailpoint);

        this.controlPoints = SuperMap.Plot.PlottingUtil.clonePoints(geoPts);
        this.scaleValues = [];
        for(var i = 0; i < scaleValueBack.length; i++){
            this.scaleValues.push(scaleValueBack[i]);
        }
    ...
    modifyPoint: function(index, pt) {
        if(pt.isScalePoint === true){
            var shapePnts = SuperMap.Plot.PlottingUtil.clonePoints(this.controlPoints);
            var nCount = shapePnts.length;
            if (nCount < 2){
                return;
            }
            ...
            if(this.scaleValues.length < 5){
                ...
                this.scaleValues.push(this.sv_arrowTailWidth);
            }

            var ptfirst = shapePnts[0].clone();
            var dAllDistance = SuperMap.Plot.PlottingUtil.polylineDistance(shapePnts);
            var ArrowBodyLen = null;
            if(ArrowBodyLen == 0.0){
                return;
            }

            var dTailWidth = dAllDistance*this.scaleValues[4];
            var sidepoint = SuperMap.Plot.PlottingUtil.getSidePointsOfLine(dTailWidth, shapePnts[1], shapePnts[0]);

            shapePnts = [];
            shapePnts.push(sidepoint.pntLeft);
            shapePnts.push(sidepoint.pntRight);
            for(var i = 1; i < this.controlPoints.length; i++){
                shapePnts.push(this.controlPoints[i].clone());
            }

            //老军标算法计算比例点
            //控制点个数
            var ctrlPntCount = shapePnts.length;

            //处理输入的控制点
            var OpectrlPoints = this.operateCtrlPts(shapePnts);
            ...
            //比例值个数
            var svCount = this.calcScaleValueCount(shapePnts.length);
            svCount++;
            if(this.scaleValues.length < svCount){
                ...
                ArrowTailwidth = this.sv_arrowTailWidth;;
            }else{
                ...
                ArrowTailwidth = this.scaleValues[4];
            }
            var ArrowBodyLen = SuperMap.Plot.PlottingUtil.polylineDistance(shapePnts);
            ...
            if(index == 1){
                ...
                ArrowTouLenDiviedByArrowBodyLen = ArrowTouLen/ArrowBodyLen;
            } else if(index == 2){//第三比例点发生变化时
                ArrowTouLenDiviedByArrowBodyLen = ArrowTouLen/ArrowBodyLen;
            }else{
                //计算箭尾宽度
                var dtailwidth = SuperMap.Plot.PlottingUtil.distance(pt,ptfirst);
                ArrowTailwidth = dtailwidth/dAllDistance;
            }
            ...
            this.scaleValues[4]= ArrowTailwidth;
        }
