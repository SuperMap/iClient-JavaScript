...
describe('maplibregl_WebMapV2', () => {
  ...
  it('test baseLayer layers count maploaded', (done) => {
    spyOn(FetchRequest, 'get').and.callFake((url) => {
      ...
      if (url.indexOf('%E4%BA%AC%E6%B4%A5%E5%9C%B0%E5%8C%BA%E5%9C%B0%E5%9B%BE.json') > -1) {
        return Promise.reject(new Error('TIle service error'));
      }
  ...
  it('when uncommon crs was defined, dont set repeat', (done) => {
    ...
    const epsgCode = 'EPSG:4217';
    const wkt_4217 = 'GEOGCS["Beijing 1954",DATUM["Beijing_1954",SPHEROID["Krassowsky 1940",6378245,298.3],TOWGS84[15.8,-154.4,-82.3,0,0,0,0]],PRIMEM["Greenwich",0,AUTHORITY["EPSG","8901"]],UNIT["degree",0.0174532925199433,AUTHORITY["EPSG","9122"]],AUTHORITY["EPSG","4217"]]';
    mapInfo.projection = wkt_4217;
    spyOn(FetchRequest, 'get').and.callFake((url) => {
      if (url.indexOf('portal.json') > -1) {
        return Promise.resolve(new Response(JSON.stringify(iportal_serviceProxy)));
      }
        ...
        return Promise.resolve(
          new Response(
            JSON.stringify({
              ...
              baseLayer: {
                ...mapInfo.baseLayer,
                url: 'http://fake:8090/iserver/services/map-test4241/rest/maps/4217_123'
              }
            })
          )
        );
        ...
        return Promise.resolve(
          new Response(
            JSON.stringify({
              ...
              baseLayer: {
                ...mapInfo.baseLayer,
                url: 'http://fake:8090/iserver/services/map-test4241/rest/maps/4217_456'
              }
            })
          )
        );
      }
      if (url.indexOf('4217_123.json') > -1) {
        return Promise.resolve(
          new Response(
            JSON.stringify({
              prjCoordSys: { epsgCode: 4217 },
              bounds: {
                top: 85,
                left: -180,
                bottom: -85,
      ...
      if (url.indexOf('4217_456.json') > -1) {
        return Promise.resolve(
          new Response(
            JSON.stringify({
              prjCoordSys: { epsgCode: 4217 },
    ...
    datavizWebmap.once('mapcreatesucceeded', ({ map: map1 }) => {
      ...
      expect(map1.getCRS().getWKT()).toBe(wkt_4217);
      ...
      datavizWebmap.on('mapcreatesucceeded', ({ map: map2 }) => {
        ...
        expect(map2.getCRS().getWKT()).toBe(wkt_4217);
  ...
  it('when uncommon crs was defined, baselayer is TILE', (done) => {
    const mapInfo = JSON.parse(raster4490);
    const epsgCode = 'EPSG:4218';
    mapInfo.projection = epsgCode;
    const wkt_4218 = 'GEOGCS["Beijing 1954",DATUM["Beijing_1954",SPHEROID["Krassowsky 1940",6378245,298.3],TOWGS84[15.8,-154.4,-82.3,0,0,0,0]],PRIMEM["Greenwich",0,AUTHORITY["EPSG","8901"]],UNIT["degree",0.0174532925199433,AUTHORITY["EPSG","9122"]],AUTHORITY["EPSG","4218"]]';
    spyOn(FetchRequest, 'get').and.callFake((url) => {
      if (url.indexOf('portal.json') > -1) {
        return Promise.resolve(new Response(JSON.stringify(iportal_serviceProxy)));
      }
      ...
      if (url.indexOf('4218_123.json') > -1) {
        return Promise.resolve(
          new Response(
            JSON.stringify({
              prjCoordSys: { epsgCode: 4218 },
              bounds: {
                top: 85,
                left: -180,
                bottom: -85,
      ...
      if (url.indexOf('prjCoordSys.wkt')) {
        return Promise.resolve(new Response(wkt_4218));
      }
    });
    const originCrs = maplibregl.CRS.get(epsgCode);
    const crsSetSpy = spyOn(maplibregl.CRS, 'set').and.callThrough();
    ...
    datavizWebmap.on('mapcreatesucceeded', ({ map }) => {
      expect(originCrs).toBeFalsy();
      expect(maplibregl.CRS.get(epsgCode)).toBeTruthy();
      expect(maplibregl.CRS.get(epsgCode)).toEqual(map.getCRS());
      expect(crsSetSpy).toHaveBeenCalledTimes(2);
      expect(map.getCRS().getEpsgCode()).toBe(epsgCode);
      expect(map.getCRS().getWKT()).toBe(wkt_4218);
  ...
  it('when uncommon crs was defined, baselayer is MAPBOXSTYLE', (done) => {
    const epsgCode = 'EPSG:4219';
    const wkt_4219 = 'GEOGCS["Beijing 1954",DATUM["Beijing_1954",SPHEROID["Krassowsky 1940",6378245,298.3],TOWGS84[15.8,-154.4,-82.3,0,0,0,0]],PRIMEM["Greenwich",0,AUTHORITY["EPSG","8901"]],UNIT["degree",0.0174532925199433,AUTHORITY["EPSG","9122"]],AUTHORITY["EPSG","4219"]]';
    const mapInfo = {
    ...
    datavizWebmap.on('mapcreatesucceeded', ({ map }) => {
      ...
      expect(crsSetSpy).toHaveBeenCalledTimes(2);
      expect(map.getCRS().getEpsgCode()).toBe(epsgCode);
      expect(map.getCRS().getWKT()).toBe(wkt_4219);
