...
import { GeoprocessingService } from '../../../src/maplibregl/services/GeoprocessingService';

const serverUrl = 'http://localhost:8090/iserver/services/geoprocessing/restjsr/gp/v2';
describe('maplibregl_GeoprocessingService', () => {
    let originalTimeout;
    let serverResult;
    beforeEach(() => {
        ...
        executeService.execute(
            ...
            (result) => {
                ...
                try {
                    ...
                    expect(serverResult.type).toBe('processCompleted');
                    expect(serverResult.result).not.toBeNull();
                    expect(serverResult.result['countrdd-resultCount']).not.toBe(12);
    ...
    it('getJobsParameter', (done) => {
        ...
        getJobsParameterService.getJobs('sps.WorkflowProcessFactory.models:sps', (result) => {
            ...
            try {
                ...
                expect(serverResult.type).toBe('processCompleted');
                expect(serverResult.result).not.toBeNull();
                const results = serverResult.result;
