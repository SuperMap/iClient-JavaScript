...
describe('ServerGeometry', () => {
    ...
    it('transformGEOCOMPOUND', () => {
        ...
        const serverGeometry = new ServerGeometry(options);
        var geocompound = serverGeometry.transformGeoCompound();
        expect(geocompound).not.toBeNull();
        expect(geocompound.CLASS_NAME).toEqual('SuperMap.Geometry.Collection');
        expect(geocompound.components.length).toEqual(2);
        var component = geocompound.components[0];
        expect(component.CLASS_NAME).toEqual('SuperMap.Geometry.LineString');
        expect(component.componentTypes[0]).toEqual('SuperMap.Geometry.Point');
        var components = component.components;
        expect(components.length).toEqual(2);
        expect(components[0].type).toEqual('Point');
        expect(components[0].x).toBe(121.73122268650864);
        expect(components[0].y).toBe(39.124988937731445);
    });
