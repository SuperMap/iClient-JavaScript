...
describe('WebPrintingService', () => {
  ...
  // hasSummaryText  hasTime为true时,不传SummaryText Time,也需要null填充
  it('_processParams_SummaryTextAndTime_Undefined', (done) => {
    var webPrintingService = new WebPrintingService(url, { crossOrigin: false });
    spyOn(webPrintingService, 'getLayoutTemplates').and.callFake(() => {
      return Promise.resolve({ result: getLayoutsResultJson });
    });
    ...
    webPrintingService._processParams(params).then(() => {
      expect(params.layoutOptions.time).toBeNull();
      expect(params.layoutOptions.summaryText).toBeNull();
      webPrintingService.destroy();
      done();
    });
  });

  // hasSummaryText  hasTime为true时,SummaryText Time空字符串是合理的
  it('_processParams_SummaryTextAndTime_emptyString', (done) => {
    var webPrintingService = new WebPrintingService(url, { crossOrigin: false });
    spyOn(webPrintingService, 'getLayoutTemplates').and.callFake(() => {
      return Promise.resolve({ result: getLayoutsResultJson });
    });
    ...
    webPrintingService._processParams(params).then(() => {
      expect(params.layoutOptions.time).toBe('');
      expect(params.layoutOptions.summaryText).toBe('');
      webPrintingService.destroy();
      done();
    });
  });
