...
function createWebMapBaseInstance() {
  const WebMapBase = createWebMapBaseExtending(Events, { mapRepo: {} });
  return new WebMapBase('', { target: 'map' });
}
...
describe('WebMapBase - getLegendInfos', () => {
  let instance;
  let mockHandler;

  beforeEach(() => {
    instance = createWebMapBaseInstance();
    mockHandler = {
      _getLegendInfos: jasmine.createSpy('_getLegendInfos').and.returnValue([
  ...
  it('should return empty array when handler._getLegendInfos returns empty array', () => {
    mockHandler._getLegendInfos.and.returnValue([]);

    expect(instance.getLegendInfos()).toEqual([]);
  });
  ...
  it('should return empty array when handler is undefined', () => {
    instance._handler = undefined;

    expect(instance.getLegendInfos()).toEqual([]);
  });

  it('should return legend info from WebMapV2-like handler', () => {
    const webMapV2Handler = {
      ...
      _mapInfo: {
        layers: [
          ...
          { name: 'Layer3', layerID: 'layer3', legendSetting: {} }
        ]
      }
    };
  ...
  it('should return legend info from WebMapV3-like handler', () => {
